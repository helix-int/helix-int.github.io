...
(function ($) {
	...
	$('#rt_theme_reset_settings input').on('click', function (e) {
		var save_control_confirm = confirm(rtframework_variables.reset_theme);
	...
	function rt_shortcode_helper(event) {
		if ($('#rttheme_shortcode_helper').length == 0) {
			...
			var data = 'action=my_action&shortcode_helper=true';
			...
			$('#rttheme_shortcode_helper').show('fade', {
				duration: 300,
				easing: 'swing',
			});
		}
		...
		function start_helper() {
			...
			if ('undefined' != typeof tinyMCE) {
				if (null != window.tinyMCE.activeEditor) {
					if (
						'' != window.tinyMCE.activeEditor.editorId &&
						'rt_hidden_rich_editor' !=
							window.tinyMCE.activeEditor.editorId
					) {
						$('#rttheme_shortcode_helper .insert_to_editor').css({
							display: 'block',
						});
					} else {
						$('#rttheme_shortcode_helper .insert_to_editor').css({
			...
			$('.insert_to_editor').on('click', function () {
				...
				if (
					$('#' + tinymce.activeEditor.id)
						.parents('#wp-content-wrap')
						.hasClass('html-active') === false
				) {
					//check if html tab is not active
					// replace new lines with <br /> if the line ends with a bracket ]

					var shortcode_lines = shortcode.split(/\n/);
					for (var i = 0, l = shortcode_lines.length; i < l; i++) {
						new_shortcode = new_shortcode + shortcode_lines[i];
						var this_line = $.trim(shortcode_lines[i]);
						var this_last_char = this_line.substr(this_line.length - 1);
			...
			if ('undefined' != typeof tinymce) {
				if ('undefined' != typeof tinymce.create) {
					...
					tinymce.PluginManager.add(
						'rt_themeshortcode',
						tinymce.plugins.rt_theme_shortcodes
					);
				}
			}
	...
	$('#wp-admin-bar-rt_shortcode_helper_button').on(
		'click',
		{ purpose: 'admin_bar' },
		rt_shortcode_helper
	);
	...
	$('.style_parts > div').on('mouseenter mouseleave', function (event) {
		...
		var tooltip_div = $(
			'<div class="rt_tooltip_message">' + tooltip_text + '</div>'
		);
		...
		if (event.type == 'mouseenter') {
			...
			tooltip_div.css({
				top: $(this).offset().top - tooltip_div.height() / 2,
				left: $(this).offset().left - 240,
			});
		}
	});
	...
	$(document).on('click', '.rttheme_upload_button', function (e) {
		...
		var this_image_holder = $(
			'[data-holderid="' + $(this).data('inputid') + '"]'
		);
	...
	//auto select
	$('.upload_field').on('focus', function () {
		$(this).trigger('select');
	});
	...
	$(document).on('click', '.rt_gallery_add_button', function (e) {
		...
		custom_uploader.on('select', function () {
			...
			var list_array =
				list_value == '' ? new Array() : list_value.split(',');
			...
			selection.map(function (attachment) {
				...
				if (
					list_array.length == 0 ||
					$.inArray(attachment.id.toString(), list_array) === -1
				) {
					$('.rt-gallery-uploaded-photos').append(
						'<li><img src="' +
		...
		custom_uploader.on('open', function () {
			...
			if ($('#rt-gallery-images').val() == '') {
				return false;
			}

			var ids = $('#rt-gallery-images').val().split(',');
			var attachment = '';
	...
	//start sortables
	$('ul.rt-gallery-uploaded-photos').sortable({
		handle: 'img',
	...
	$(document.body).on(
		'click',
		'.rt-gallery-uploaded-photos li .gallery_delete',
		function () {
			...
			var item_to_delete = this_image_holder
				.find('img:eq(0)')
				.attr('data-rel');
	...
	$(document).on('change', '.div_controller', function () {
		...
		var options_set = $(this)
			.parents('.options_set_holder:eq(0)')
			.find('.hidden_options_set:eq(0)');

		if (
			selected_option === 'new' ||
			selected_option === 'boxed-body' ||
			selected_option === 'half-boxed' ||
			selected_option === 'disabled_parallax'
		) {
	...
	function rt_icon_selection(event) {
		var purpose = event.data.purpose;
		var thisField = $(event.target);
		var iconSelector = $('.icon-selection');
		...
		if (iconSelector.length == 0) {
			$('<div class="rt_loading_bar"></div>').appendTo('body'); //create loading bar
			var data = 'action=my_action&iconSelector=true';
			$.post(ajaxurl, data, function (response) {
				$('.rt_loading_bar').remove();
		...
		$(document.body).on('click', '.list-icons li', function () {
			if (purpose == 'item') {
				...
				for (i = 0; i < classNames.length; i++) {
					if (
						classNames[i].search(/icon-/i) == 0 &&
						selectted_icon_name == 'blank'
					) {
						//found & deleted
						newclassNames += '';
					} else if (
						...
						// not found & added
						newclassNames +=
							' ' + classNames[i] + ' ' + selectted_icon_name;
	...
	$(document.body).on(
		'click',
		'.button_icon,.button2_icon,.icon_name,.icon_selection,.edit-menu-item-classes,[data-setting="rt-icon-name"]',
		{ purpose: 'item' },
		rt_icon_selection
	);
	$('#wp-admin-bar-rt_icons .ab-item div').on(
		'click',
		{ purpose: 'admin_bar' },
		rt_icon_selection
	);
				...
				if (settings.data) {
					if (
						...
						(settings.data.search('id_base=' + widget_id_base) != -1 ||
							settings.data.search('id_base=' + widget_id_base_2) !=
								-1 ||
							settings.data.search('id_base=' + widget_id_base_3) !=
								-1 ||
							settings.data.search('id_base=' + widget_id_base_4) != -1)
					) {
						...
						jQuery(
							'select[multiple]#widget-' + thisWidtedID + '-categories'
						).asmSelect({
							addItemTarget: 'bottom',
							animate: true,
							highlight: true,
	...
	$(document).on('click', '#rt_theme_custom_fonts .delete-font', function () {
		...
		var delete_font_confirm = confirm(rtframework_variables.delete_font);
	...
	$.fn.extend({
		ShowPostFormats: function () {
			...
			$(post_formats[theSelectedFormat]).show().effect('highlight', 700);
		},
	});

	$('#post-formats-select input:checked').ShowPostFormats();

	$('#post-formats-select').on('change', function (event) {
		$('#post-formats-select input:checked').ShowPostFormats();
	});
	...
	$(window).on('load', function () {
		if ($('#rttheme_portfolio_post_format input:checked').length > 0) {
			$(
				'#rttheme_portfolio_post_format input:checked'
			).rt_portfolio_formats();
		} else {
			$('#rttheme_portfolio_post_format-1')
				.attr('checked', true)
				.rt_portfolio_formats();
		}

		$('#rttheme_portfolio_post_format').on('change', function (event) {
			$(
				'#rttheme_portfolio_post_format input:checked'
			).rt_portfolio_formats();
		});
	...
	if ($('body').hasClass('nav-menus-php')) {
		...
		$('.menu-item-settings').each(function () {
			var classNames = $(this).find('.edit-menu-item-classes').val();

			if (classNames.search(/multicolumn-/i) > -1) {
				$(this).find('.field-move').before(form_checked);

				//find the stored column size
				var classNames_split = classNames.split(' ');
				var stored_column_count = '';

				for (i = 0; i < classNames_split.length; i++) {
					if (classNames_split[i].search(/multicolumn-/i) > -1) {
						var stored_column_count_array =
							classNames_split[i].split('-');
						stored_column_count =
							stored_column_count_array[
								stored_column_count_array.length - 1
							];
					}
				}
				...
				$(this)
					...
					.before(
						column_count
							.replace(/column-item-size-value/i, stored_column_count)
							.replace(/hidden-field/i, '')
					);
			} else {
				$(this).find('.field-move').before(form);
				$(this)
					.find('.field-move')
					.before(column_count.replace(/column-item-size-value/i, '3'));
			}
		});
				...
				$('.menu-item.pending').each(function () {
					if ($(this).find('.rt-multi-column-menu').length < 1) {
						...
						$(this)
							.find('.field-move')
							.before(
								column_count.replace(/column-item-size-value/i, '3')
							);
					}
				});
			});
		...
		$(document.body).on('click', '.rt-multi-column-menu', function () {
			...
			var column_count_holder = $(this)
				.parents('.rt-mega-menu:eq(0)')
				.next('.rt-mega-menu');
			var column_count = column_count_holder.find(
				'.rt-multi-column-item-size'
			);

			if ($(this).prop('checked')) {
				css_field.val(
					$.trim(css_class_value) + ' multicolumn-' + column_count.val()
				);
				...
				for (i = 0; i < classNames.length; i++) {
					if (classNames[i].search(/multicolumn-/i) > -1) {
						//found & deleted
						newclassNames += '';
					} else {
						newclassNames += classNames[i];
	...
	********************************************************************************** */
	$('#rt-demo-parts').on('change', function () {
		if (
			$('option:selected', $(this)).val() == 'all' ||
			$('option:selected', $(this)).val() == 'contents'
		) {
	...
	$('#rt-demo-import-button').on('click', function (e) {
		...
		var button = $(this),
			...
			selected_builder = $('option:selected', $('#rt-builder-option')),
			contents_result = $('#contents-result'),
			widgets_result = $('#widgets-result'),
			options_result = $('#options-result'),
			is_revslider_installed = $(
				'#rt-demo-parts option[value="revslider"]'
			).data('active'),
		...
		if (selected_parts == 'all' || selected_parts == 'contents') {
			if (
				$('#rt-builder-option').length > 0 &&
				selected_builder.val() == ''
			) {
				alert(rtframework_params['select_builder']);
				$('#rt-builder-option').effect('highlight', 3000);
				return;
			}

			if (!selected_builder.data('active')) {
				if ('elementor' == selected_builder.val()) {
					alert(rtframework_params['install_elementor']);
				}

				if ('vc' == selected_builder.val()) {
					alert(rtframework_params['install_vc']);
				}
			...
			$.ajax({
				...
				success: function (response, textStatus, XMLHttpRequest) {
					...
					if (response.indexOf('{{importer_error}}') >= 0) {
						alert(
							'Import process cancelled, click to the logs link to see the errors.'
						);
						contents_result
							.find('.logs')
							.append(response.replace('{{importer_error}}', ''));
						contents_result
							.removeClass('importing')
							.addClass('import-failed');
						$(window)
							.trigger('rt_contents_imported')
							.off('rt_contents_imported');
						return false;
					}
		...
		if (selected_parts == 'all' || selected_parts == 'contents') {
			...
			contents_result
				.removeClass('imported import-failed')
				.addClass('importing');
		...
		if (
			is_revslider_installed &&
			(selected_parts == 'all' || selected_parts == 'revslider')
		) {
	...
	$('#rt-demo').on('change', function (e) {
		...
		if (demo == '') {
			...
			demo_image
				.attr('src', demo_image_base_url + '/images/skins/' + demo + '.png')
				.show();
		}
	});
	...
	********************************************************************************** */
	$('[data-vc-ui-element="button-save"]').on('click', function (e) {
		$('.vc_ui-panel-content-container')
			.find('[data-rt-group]')
			.each(function () {
				var $this = $(this);
				var param_name = $this.data('rt-group');
				var params = $(this)
					.parent()
					.find('[data-rt-' + param_name + ']');
				var value = '';
				var counter = 1;

				params.each(function () {
					value += $(this).val();
					value += counter !== params.length ? ',' : '';
					counter++;
				});
				$this.attr('value', value);
			});
	...
	/* ******************************************************************************* 
		...
		VC Backend only elements

	********************************************************************************** */
	$(
		'[data-element="rt_tabs"],[data-element="rt_accordion"],[data-element="rt_slider"],[data-element="rt_pricing_table"],[data-element="rt_compare_table"],[data-element="rt_timeline"],[data-element="rt_icon_list"],[data-element="rt_image_gallery"],[data-element="google_maps"]'
	).on('click', function (e) {
		if ($('.vc_navgar-frontend').length < 1) {
			return;
		}

		e.preventDefault();
		alert(rtframework_variables.vcbackend_only);
		return false;
	});
	...
	********************************************************************************** */
	$('#rt-framework-license-check').on('click', function (e) {
		var button = $(this),
			licenseKey = $('#rt-framework-license-key').val(),
			plugin_status_bar = $('.rt-framework-installer-status.plugin'),
			loader = button
				.parent('.rt-framework-installer-bottom')
				.find('.rt-framework-installer-loading');

		loader.show();
		button.hide();

		plugin_status_bar.html(rtframework_params.checking_license);

		$.ajax({
			type: 'POST',
			url: rtframework_params.ajax_url,
			data: {
				action: 'rtframework_license_manager',
				'wpnonce-manager': button.data('wpnonce-manager'),
			...
			success: function (response, textStatus, XMLHttpRequest) {
				if (response.error) {
					loader.hide();
					button.show();
					plugin_status_bar.html(response.error);
				} else {
					loader.hide();
					button.hide();
					plugin_status_bar.html(response.ok);
			...
			error: function (MLHttpRequest, textStatus, errorThrown) {
				loader.hide();
				button.show();
				plugin_status_bar.html(rtframework_params.license_error);
				console.log(errorThrown);
			},
		});
	});
	...
	********************************************************************************** */
	$('#rt-framework-install-plugins-button').on('click', function (e) {
		var button = $(this),
			plugins = [],
			plugins__install = [],
			plugins__activate = [],
		...
		$('.rt-framework-installer-plugin-list.checked:not(.done)').each(
			function () {
				var plugin_slug = $(this).data('plugin-slug');
				var plugin_action = $(this).data('plugin-action');
				...
				if (
					...
					plugins__install.push(plugin_slug);
				} else if (
					plugin_action == 'update' ||
					plugin_action == 'update-activate'
				) {
					plugins__update.push(plugin_slug);
				}
				...
				if (
					...
					plugins__activate.push(plugin_slug);
				}
			}
		);

		var install_task = {
			plugin: plugins__install,
			action: 'tgmpa-bulk-install',
			action2: -1,
			_wpnonce: button.data('wpnonce-plugin'),
			plugin_status: 'all',
			'tgmpa-page': button.data('plugins-page'),
			url: button.data('plugins-url'),
			plugin_status: 'all',
			task: 'install',
		};

		var update_task = {
			plugin: plugins__update,
		...
		if (plugins__install.length) {
			tasks.push('install_task');
		}

		if (plugins__update.length) {
			tasks.push('update_task');
		}

		if (plugins__activate.length) {
			tasks.push('activate_task');
		}

		//start
		if (tasks.length) {
			rt_framework_manage_plugins(eval(tasks[0]));
			loader.show();
			button.hide();
		} else {
			alert(rtframework_params.select_task);
		}

		var rt_framework_plugin_status_array = [];

		function rt_framework_plugin_status() {
			$.ajax({
				type: 'POST',
				url: rtframework_params.ajax_url,
				data: {
					action: 'rtframework_plugin_manager',
					action_2: 'plugin_status',
					'wpnonce-manager': button.data('wpnonce-manager'),
					plugins: button.data('plugins'),
				},

				success: function (response, textStatus, XMLHttpRequest) {
					rt_framework_plugin_status_array = response;
					$(window).trigger('rt_framework_plugin_status_ready');
				},
				error: function (MLHttpRequest, textStatus, errorThrown) {
					loader.hide();
					button.show();
					plugin_status_bar.html(rtframework_params.error_plugins);
					console.log(errorThrown);
				},
			});
		}

		function rt_framework_manage_plugins(data) {
			if (tasks[0] == 'install_task') {
				plugin_status_bar.html(rtframework_params.installing_plugins);
			} else if (tasks[0] == 'update_task') {
				plugin_status_bar.html(rtframework_params.updating_plugins);
			} else if (tasks[0] == 'activate_task') {
				plugin_status_bar.html(rtframework_params.activating_plugins);
			}
			...
			$.ajax({
				...
				success: function (response, textStatus, XMLHttpRequest) {
					...
					if (tasks.length) {
						...
						rt_framework_plugin_status();
					}
				},
				error: function (MLHttpRequest, textStatus, errorThrown) {
					loader.hide();
					button.show();
					plugin_status_bar.html(
						'<span style="color:red;">' +
							rtframework_params.error_plugins +
							' [error code: ' +
							tasks[0] +
							'_error]</span>'
					);
		...
		$(window).off('rt_framework_plugin_status_ready');
		$(window).on('rt_framework_plugin_status_ready', function () {
			var error = false;

			$(plugins__install).each(function () {
				if (rt_framework_plugin_status_array[this].installed) {
					$('[data-plugin-slug="' + this + '"]').addClass(
						'done installed'
					);
				} else {
					$('[data-plugin-slug="' + this + '"]').addClass(
					...
					error = this + '-install-failed';
				}
			});

			$(plugins__update).each(function () {
				if (rt_framework_plugin_status_array[this].updated) {
					$('[data-plugin-slug="' + this + '"]').addClass('done updated');
				} else {
					$('[data-plugin-slug="' + this + '"]').addClass('error updated');
					error = this + '-update-failed';
				}
			});

			$(plugins__activate).each(function () {
				if (rt_framework_plugin_status_array[this].activated) {
					$('[data-plugin-slug="' + this + '"]').addClass(
						'done activated'
					);
				} else {
					...
					error = this + '-activate-failed';
				}
			});

			if (!error) {
				plugin_status_bar.html(rtframework_params.done);
				loader.hide();
				button.remove();
				...
				button.show();
				plugin_status_bar.html(
					rtframework_params.error_plugins + ' [error code: ' + error + ']'
				);
			}
		});
	});

	$('.rt-framework-installer-plugin-list').on('click', function (e) {
