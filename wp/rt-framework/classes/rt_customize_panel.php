...
class RTFramework_Customize_Panel extends RTFramework
{
	 ...
	 * Utility options
	 */
	public $utility_options = array('businesslounge_optimize_css'=>'on','businesslounge_optimize_js'=>'on','businesslounge_404_page'=>'','businesslounge_maintenance_mode'=>'', 'businesslounge_maintenance_page'=>'', 'is_portfolio_active' => 'on', 'is_team_active' => 'on', 'is_testimonials_active' => 'on' );
	...
	public function load_fonts()
	{
			...
			$rt_custom_fonts = unserialize( get_option( "businesslounge_custom_fonts" ) );
		
			if( ! empty( $rt_custom_fonts ) ){

				$this->fonts["#2_obt_start"] = esc_html_x("Custom Fonts",'Admin Panel','businesslounge');
			...
			$this->fonts["#1_obt_start"] = esc_html_x("Web Safe Fonts",'Admin Panel','businesslounge');
			...
			$google_fonts = array();
				...
				if ( $json ){
					...
					$this->fonts["#3_obt_start"] = esc_html_x("Google Fonts",'Admin Panel','businesslounge');
	...
	public function customize_admin_js() {
		...
		//localize js params
		$js_params = array(
			"apply_skin" => esc_html_x('Do you want to apply this skin? ','Admin Panel','businesslounge'), 
			"select_parts" => esc_html_x('Please select a demo part to install.','Admin Panel','businesslounge'), 
			"select_demo" => esc_html_x('Please select a demo.','Admin Panel','businesslounge'), 
			"select_builder" => esc_html_x('Select a builder to proceed.','Admin Panel','businesslounge'), 
			"install_elementor" => esc_html_x('Install and activate Elementor plugin to proceed.','Admin Panel','businesslounge'), 
			"install_revslider" => esc_html_x('Install and activate Revolution Slider plugin to proceed.','Admin Panel','businesslounge'), 
			"install_vc" => esc_html_x('Install and activate Visual Composer plugin to proceed.','Admin Panel','businesslounge'), 
			"wait_previous_install" => esc_html_x('An installer has already been started. Please wait!','Admin Panel','businesslounge'), 
			"theme_slug" => RT_THEMESLUG,
			"ajax_url" => admin_url('admin-ajax.php'),
			"installing_plugins" => esc_html_x('Installing plugins.','Admin Panel','businesslounge'),
			"updating_plugins" => esc_html_x('Updating plugins.','Admin Panel','businesslounge'),
			"activating_plugins" => esc_html_x('Activating plugins.','Admin Panel','businesslounge'),
			"done" => esc_html_x('Done.','Admin Panel','businesslounge'),
			"error_plugins" => esc_html_x('An error occurred while processing the tasks.','Admin Panel','businesslounge'),
			"select_task" => esc_html_x('Select tasks to apply.','Admin Panel','businesslounge'),
			"checking_license" => esc_html_x('Confirming the license key. Please wait!','Admin Panel','businesslounge'),
			"license_error" =>  esc_html_x('License activation failed.','Admin Panel','businesslounge') 			
		);

		wp_localize_script( 'customizer-rt-skins-js', 'rtframework_params', $js_params );
	}
	...
	public function save_defaults( $reset = "" )
	{
		...
		$is_defaults_saved = get_option('businesslounge_'.RT_UTHEME_NAME.'_defaults'); 
		...
		//add a db note
		update_option('businesslounge_'.RT_UTHEME_NAME.'_defaults','saved'); 
