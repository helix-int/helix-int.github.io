...
if( ! function_exists("rtframework_logo_function") ){
	...
	function rtframework_logo_function(){
			...
			//initial height
			$initial_height_dark = ! empty( $logo_sets["logo_dark"]["url"] ) ? ' style="--initial-height:'. esc_attr( $logo_sets["logo_dark"]["url"][2] ).'px"' : "";
			$initial_height_light = ! empty( $logo_sets["logo_light"]["url"] ) ? ' style="--initial-height:'. esc_attr( $logo_sets["logo_light"]["url"][2] ).'px"' : "";

			//retina srcsets
			$logo_dark_srcset = ! empty( $logo_sets["retina_logo_dark"]["url"] ) ? $initial_height_dark . ' srcset="'.$logo_sets["retina_logo_dark"]["url"][0].' 1.3x"' : ""; 
			$logo_light_srcset = ! empty( $logo_sets["retina_logo_light"]["url"] ) ? $initial_height_light . ' srcset="'.$logo_sets["retina_logo_light"]["url"][0].' 1.3x"' : ""; 		 
...
if( ! function_exists("rtframework_sticky_logo_function") ){
	...
	function rtframework_sticky_logo_function(){
			...
			//initial height
			$initial_height = ! empty( $sticky_logo ) ? ' style="--initial-height:'. min(60,esc_attr( $sticky_logo[2] )).'px"' : "";

			//retina srcsets
			$logo_srcset = ! empty( $retina_sticky_logo ) ? $initial_height . ' srcset="'.$retina_sticky_logo[0].' 1.3x"' : "";  
...
if( ! function_exists("rtframework_mobile_logo_function") ){
	...
	function rtframework_mobile_logo_function(){
			...
			//initial height
			$initial_height = ! empty( $mobile_logo ) ? ' style="--initial-height:'. min(100,esc_attr( $mobile_logo[2] )).'px"' : "";

			//retina srcsets
			$logo_srcset = ! empty( $retina_mobile_logo ) ? $initial_height .' srcset="'.$retina_mobile_logo[0].' 1.3x"' : "";  
...
if ( ! function_exists("rtframework_save_elementor_defaults") ){
	...
	function rtframework_save_elementor_defaults(){
		...
		if( ! class_exists("Elementor\Plugin") ){
			...
			$_elementor_page_settings = get_post_meta( $elementor_active_kit, '_elementor_page_settings' );

			$new_settings = array();
			$new_settings['system_colors'] = [];
			$new_settings['system_colors'][] = [ '_id' => 'primary', 'title' => 'Primary', 'color' => '#b9b9b9'];
			$new_settings['system_colors'][] = [ '_id' => 'secondary', 'title' => 'Secondary', 'color' => '#000000'];
			$new_settings['system_colors'][] = [ '_id' => 'text', 'title' => 'Text', 'color' => '#777777'];
			$new_settings['system_colors'][] = [ '_id' => 'accent', 'title' => 'Accent', 'color' => '#CE1B28'];
	
			$new_settings['system_typography'] = [];
			$new_settings['system_typography'][] = [ '_id' => 'primary', 'title' => 'Primary', 'typography_typography' => 'custom'];
			$new_settings['system_typography'][] = [ '_id' => 'secondary', 'title' => 'Secondary', 'typography_typography' => 'custom'];
			$new_settings['system_typography'][] = [ '_id' => 'text', 'title' => 'Text', 'typography_typography' => 'custom'];
			$new_settings['system_typography'][] = [ '_id' => 'accent', 'title' => 'Accent', 'typography_typography' => 'custom'];
	
			$new_settings['container_width'] = ['unit' => 'px', 'size' => 1240 ];
			$new_settings['site_name'] = get_bloginfo('name');
			
	
			update_post_meta($elementor_active_kit, '_elementor_page_settings', $new_settings ); 

			Elementor\Plugin::$instance->files_manager->clear_cache();	   

	}
} 
add_action( 'rt_after_content_import', 'rtframework_save_elementor_defaults', 10 );
...
if( ! function_exists("businesslounge_version_1_9_6_fixes") ){
	/**
	 *
	 * v1.9.6 Fixes
	 * Perform fixes required for v1.9.6
	 *
	 */
	function businesslounge_version_1_9_6_fixes($old_version,$new_version){

		if( version_compare($old_version,"1.9.6","<") && version_compare($new_version,"1.9.6",">=") ){
			update_option('elementor_experiment-e_dom_optimization','inactive');
		}

	}
}
add_action( 'rtframework_theme_updated' , 'businesslounge_version_1_9_6_fixes', 10, 2);
...
if( ! function_exists( "rt_framework_reset_widgets" ) ){
	/**
	 * Reset Widgets
	 * move all active widgets to the inactive state
	 */	
	function rt_framework_reset_widgets(){

		$widgets = get_option("sidebars_widgets");

		$all_active_widgets = array();

		foreach($widgets as $sidebar => $widget){
			if( is_array( $widget ) ){
				$all_active_widgets = array_merge($all_active_widgets, $widget);
			}
		}
		...
		foreach($widgets as $sidebar => $widget){
			if( $sidebar == "wp_inactive_widgets" ){		
				$new_widgets[$sidebar] = $all_active_widgets;	
			}else{
				$new_widgets[$sidebar] = [];
			}	
		}
