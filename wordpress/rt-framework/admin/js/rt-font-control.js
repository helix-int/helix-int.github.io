/*!
 *
 * businesslounge WordPress Theme Admin - Font Control
 * Copyright (C) 2014-2022 RT-Themes
 * http://rtthemes.com
 *
 */
(function ($) {
	...
	$(window).on('load', function () {
		function rt_create_option_inputs(list, selected_values) {
			...
			var selected_values_array = selected_values.toString().split(',');
			...
			for (i = 0; i < list.length; ++i) {
				selected =
					$.inArray(list[i], selected_values_array) > -1 ? 'selected' : '';
		...
		setTimeout(function () {
			$('.rt_fonts').on('change', function () {
				...
				if (kind == 'google' && subsets) {
					el.html(
						rt_create_option_inputs(
							subsets,
							$(this).data('selected-subsets')
						)
					).trigger('change');
